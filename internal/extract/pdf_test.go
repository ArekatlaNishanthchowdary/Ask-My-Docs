...
// FuzzExtractPDF exercises the recover() boundary in extractPDF directly:
// the built-in parser is documented to panic on malformed xref tables and
// broken object streams, so a corrupt upload must come back as an error, not
// a crash or a hang.
func FuzzExtractPDF(f *testing.F) {
	seeds := [][]byte{
		minimalPDF(700, 100, "TITLE LINE", "FOOTER LINE"),
		minimalPDF(700, 100, "", ""),
		[]byte("# just markdown\n"),
		[]byte("%PDF-1.4\n1 0 obj\n<</Type/Catalog"),
		{},
		[]byte("%PDF-1.7\n%%EOF\n"),
		{0x00, 0xFF, 0x10, 0x9A, 0x00, 0x01},
	}
	for _, s := range seeds {
		f.Add(s)
	}
	...
	f.Fuzz(func(t *testing.T, data []byte) {
		...
		case <-time.After(5 * time.Second):
			t.Fatal("extractPDF did not return within 5s — possible unbounded loop")
		}
	})
}
