...
// truncate shortens a string for error messages. Duplicated in rag, qdrant
// and providers too — three lines each, not worth a shared package.
...
// maxZipEntries and maxZipBudgetBytes bound the cost of one document. A single
// entry was already capped at 64MiB (below), but nothing stopped a document
// naming thousands of small parts from spending that budget thousands of
// times over, or an archive naming millions of tiny entries from making every
// scan over zr.File expensive. Both are upload-endpoint inputs, so both get a
// cap.
const (
	maxZipEntries     = 10000
	maxZipBudgetBytes = 200 << 20
)

// zipBudget accumulates cost across every zipEntry read for one document, so
// a document with many small parts (many chart parts, many slides) cannot
// spend the per-entry cap over and over into an unbounded total.
type zipBudget struct {
	bytes int64
}

// errBudgetExceeded marks a zipEntry failure as the aggregate cap firing,
// distinct from "this optional part is absent" (a missing sharedStrings.xml,
// a missing rels file, a corrupt slide). Every loop that reads many entries
// treats a plain zipEntry error as "skip this one part" — that is correct for
// a missing part, but exceeding the budget must abort extraction instead of
// being swallowed the same way, or the cap stops nothing.
var errBudgetExceeded = errors.New("zip budget exceeded")

func (b *zipBudget) spend(n int) error {
	b.bytes += int64(n)
	if b.bytes > maxZipBudgetBytes {
		return fmt.Errorf("document exceeds the %dMiB decompressed-content budget: %w",
			maxZipBudgetBytes>>20, errBudgetExceeded)
	}
	return nil
}
...
func openZip(data []byte) (*zip.Reader, error) {
	...
	if len(zr.File) > maxZipEntries {
		return nil, fmt.Errorf("archive has %d entries, over the %d limit", len(zr.File), maxZipEntries)
	}
	return zr, nil
}

func zipEntry(zr *zip.Reader, name string, budget *zipBudget) ([]byte, error) {
	for _, f := range zr.File {
		if f.Name == name {
			...
			if err := budget.spend(len(data)); err != nil {
				return nil, err
			}
			return data, nil
		}
...
func extractDocx(data []byte) (string, error) {
	...
	budget := &zipBudget{}
	body, err := zipEntry(zr, "word/document.xml", budget)
	if err != nil {
	...
	charts, err := extractCharts(zr, budget)
	if err != nil {
		return "", err
	}
...
func extractPptx(data []byte) (string, error) {
	...
	for _, s := range slides {
		body, err := zipEntry(zr, s.name, budget)
		if err != nil {
...
func extractXlsx(data []byte) (string, error) {
	...
	budget := &zipBudget{}
	wbRaw, err := zipEntry(zr, "xl/workbook.xml", budget)
	if err != nil {
	...
	if relRaw, err := zipEntry(zr, "xl/_rels/workbook.xml.rels", budget); err == nil {
		var rels xlsxRels
	...
	shared, err := sharedStrings(zr, budget)
	if err != nil {
		return "", err
	}
	...
	for i, sh := range wb.Sheets {
		...
		body, err := zipEntry(zr, name, budget)
		if err != nil {
			if errors.Is(err, errBudgetExceeded) {
				return "", err
			}
...
// sharedStrings returns nil, nil when the part is legitimately absent (a
// workbook with no shared strings is valid); it returns an error only when
// the budget itself was exceeded reading it.
func sharedStrings(zr *zip.Reader, budget *zipBudget) ([]string, error) {
	raw, err := zipEntry(zr, "xl/sharedStrings.xml", budget)
	if err != nil {
		if errors.Is(err, errBudgetExceeded) {
			return nil, err
		}
