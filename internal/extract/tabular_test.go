...
// FuzzExtractCSV covers encoding/csv edge cases (unbalanced quotes, ragged
// rows, embedded newlines) that LazyQuotes+FieldsPerRecord=-1 were chosen to
// tolerate — the assertion is only "never panic", never a specific parse.
func FuzzExtractCSV(f *testing.F) {
	seeds := []string{
		"\xEF\xBB\xBFcomponent,qty,notes\n\"Frame F450, with PCB\",4,FRAME\n",
		"a\tb\n1\t2\n",
		"",
	...
	f.Fuzz(func(t *testing.T, data []byte, comma rune) {
		if comma == 0 || comma == '\r' || comma == '\n' || comma > 0x10FFFF {
			t.Skip("not a valid csv.Reader.Comma value")
		}
		done := make(chan struct{})
		go func() {
			defer close(done)
			_, _ = extractCSV(data, comma)
		}()
		select {
		case <-done:
		case <-time.After(5 * time.Second):
			t.Fatal("extractCSV did not return within 5s — possible unbounded loop")
		}
	})
}
