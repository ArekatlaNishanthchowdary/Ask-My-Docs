...
// checked in as binaries — the test stays readable and diffable. testing.TB
// rather than *testing.T so FuzzLoadDocumentText can build seeds with it too.
func buildZip(t testing.TB, entries map[string]string) []byte {
	t.Helper()
...
<w:body>
  ...
  <w:tbl>
	...
	// Chunking along these headings is verified in package rag
	// (TestChunkDocRespectsHeadings) — this package only owns extraction.
}
...
// TestSafeCorpusPathAcceptsOfficeAndRejectsLegacy lives in package rag
// (rag_test.go) — SafeCorpusPath is rag's, not extract's.

// FuzzLoadDocumentText is the entry point every upload reaches: seeded with
// valid docx/pptx/xlsx built the same way the table tests build them, plus
// the hand-written adversarial cases from TestLoadDocumentTextRejectsBadInput.
// The only assertion is "never panic" — a wrong extraction is a bug caught
// elsewhere, a panic on attacker-controlled bytes is this test's whole job.
func FuzzLoadDocumentText(f *testing.F) {
	validDocx := buildZip(f, map[string]string{
		"word/document.xml": `<?xml version="1.0"?>
<w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main">
<w:body><w:p><w:r><w:t>Hello</w:t></w:r></w:p></w:body></w:document>`,
	})
	validPptx := buildZip(f, map[string]string{
		"ppt/slides/slide1.xml": `<?xml version="1.0"?><p:sld xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main">` +
			`<a:t>Hi</a:t></p:sld>`,
	})
	validXlsx := buildZip(f, map[string]string{
		"xl/workbook.xml": `<?xml version="1.0"?>
<workbook xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships">
...
<worksheet><sheetData><row r="1"><c r="A1"><v>1</v></c></row></sheetData></worksheet>`,
	...
	validChart := buildZip(f, map[string]string{
		"word/document.xml": `<?xml version="1.0"?>
<w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main">
<w:body><w:p><w:r><w:t>Hi</w:t></w:r></w:p></w:body></w:document>`,
		"word/charts/chart1.xml": `<?xml version="1.0"?><c:chartSpace xmlns:c="c"><c:chart><c:plotArea><c:barChart>` +
			`<c:ser><c:val><c:v>1</c:v></c:val></c:ser></c:barChart></c:plotArea></c:chart></c:chartSpace>`,
	})
	...
	seeds := []struct {
		...
		{"deck.pptx", validPptx},
		{"book.xlsx", validXlsx},
		{"chart.docx", validChart},
		{"report.doc", []byte("anything")},
		{"broken.docx", []byte("this is not a zip")},
		{"notes.md", []byte{0xff, 0xfe, 0x00}},
		{"thing.rtf", []byte("x")},
		{"empty.docx", []byte{}},
		{"a.md", []byte("# Hi\n\nthere")},
	}
	...
	f.Fuzz(func(t *testing.T, name string, data []byte) {
		...
		go func() {
			defer close(done)
			_, _ = LoadDocumentText(name, data)
		}()
		select {
		case <-done:
		case <-time.After(5 * time.Second):
			t.Fatalf("LoadDocumentText(%q) did not return within 5s — possible unbounded loop", name)
		}
	})
}
