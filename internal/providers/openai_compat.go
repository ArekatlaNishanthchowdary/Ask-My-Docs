...
func NewOpenAICompat(cfg rag.Config) *OpenAICompat {
...
func (o *OpenAICompat) Contextualize(ctx context.Context, doc string, chunks []rag.Chunk) ([]string, error) {
	...
	for start := 0; start < len(chunks); start += o.CtxBatch {
		...
		g.Go(func() error {
			...
			err := o.chat(gctx, o.Model, fmt.Sprintf(contextSystem, rag.FormatDocument(doc)),
...
func (o *OpenAICompat) Answer(ctx context.Context, question string, sources []rag.Source) (rag.Answer, error) {
	var sb strings.Builder
...
// built under.
func (o *OpenAICompat) Verify(ctx context.Context, pairs []rag.VerifyPair) ([]bool, error) {
	if len(pairs) == 0 {
		return nil, nil
	}
