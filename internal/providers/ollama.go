...
func NewOllama(cfg rag.Config) *Ollama {
	return &Ollama{
...
index you were given must appear exactly once, including the ones you score 0.` + rag.UntrustedDataNotice
...
func (o *Ollama) Rerank(ctx context.Context, query string, docs []string, topK int) ([]rag.Scored, error) {
	if len(docs) == 0 {
		return nil, nil
	}
	...
	out := make([]rag.Scored, len(docs))
	for i, s := range scores {
		out[i] = rag.Scored{Index: i, Score: s}
	}
...
func (o *Ollama) Contextualize(ctx context.Context, doc string, chunks []rag.Chunk) ([]string, error) {
	...
	for start := 0; start < len(chunks); start += o.CtxBatch {
		...
		g.Go(func() error {
			...
			var sb strings.Builder
			for i, c := range batch {
				sb.WriteString(rag.FormatChunk(i, c.Section, c.Text))
			}
			...
			err := o.chat(gctx, fmt.Sprintf(contextSystem, rag.FormatDocument(docView)),
...
func (o *Ollama) Answer(ctx context.Context, question string, sources []rag.Source) (rag.Answer, error) {
	var sb strings.Builder
	sb.WriteString(rag.FormatSources(sources))
...
func (o *Ollama) Verify(ctx context.Context, pairs []rag.VerifyPair) ([]bool, error) {
	if len(pairs) == 0 {
		return nil, nil
	}
