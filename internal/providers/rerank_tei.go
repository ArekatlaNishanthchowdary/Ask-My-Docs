...
func (t *TEIReranker) Rerank(ctx context.Context, query string, docs []string, topK int) ([]rag.Scored, error) {
	if len(docs) == 0 {
		return nil, nil
	}
	scores := make([]rag.Scored, len(docs))
	...
	for start := 0; start < len(docs); start += t.Batch {
		...
		g.Go(func() error {
			...
			for _, s := range part {
				...
				// caller's numbering.
				scores[start+s.Index] = rag.Scored{Index: start + s.Index, Score: s.Score}
			}
			return nil
		})
	}
...
func (t *TEIReranker) rerankBatch(ctx context.Context, query string, docs []string) ([]rag.Scored, error) {
...
func NewRerankedEmbedder(inner rag.Embedder, reranker *TEIReranker) RerankedEmbedder {
	return RerankedEmbedder{Embedder: inner, reranker: reranker}
}

func (r RerankedEmbedder) Rerank(ctx context.Context, query string, docs []string, topK int) ([]rag.Scored, error) {
	return r.reranker.Rerank(ctx, query, docs, topK)
}

// Unwrap exposes the wrapped embedder so EnsureCollections can probe its real
// dimension rather than this wrapper's.
func (r RerankedEmbedder) Unwrap() rag.Embedder { return r.Embedder }
