...
// aclFilter duplicates rag's query-time ACL filter shape for this test only —
// see the comment on testSparseEncode above.
...
// testSparseEncode duplicates rag.SparseEncode's tokenizing so this integration
// test can build realistic sparse vectors without importing package rag (which
// itself imports qdrant — that would cycle). Kept in sync by hand; if lexical
// matching in a subtest below ever looks wrong, check this against
// internal/rag/sparse.go first.
func testSparseEncode(s string) SparseVec {
...
func TestIntegrationQdrantRoundTrip(t *testing.T) {
	...
	pts := []Point{
		{
			ID:     PointID("doc.md#0"),
			Vector: map[string]any{"dense": target, "sparse": testSparseEncode("refunds are issued within 10 business days")},
		...
		{
			ID:     PointID("doc.md#1"),
			Vector: map[string]any{"dense": vec(2), "sparse": testSparseEncode("invoices are sent monthly ERR-4290")},
			Payload: map[string]any{
	...
	t.Run("hybrid RRF fusion returns both legs", func(t *testing.T) {
		hits, err := q.HybridSearch(ctx, coll, target, testSparseEncode("refunds business days"), 10, nil)
		if err != nil {
	...
	t.Run("sparse leg alone finds an exact identifier", func(t *testing.T) {
		...
		hits, err := q.HybridSearch(ctx, coll, target, testSparseEncode("ERR-4290"), 10, nil)
		if err != nil {
			t.Fatalf("HybridSearch: %v", err)
		}
		var found bool
	...
	t.Run("ACL filter excludes chunks the caller cannot see", func(t *testing.T) {
		hits, err := q.HybridSearch(ctx, coll, target, testSparseEncode("refunds invoices"), 10,
			aclFilter([]string{"hr"}))
		if err != nil {
			t.Fatalf("HybridSearch: %v", err)
		}
