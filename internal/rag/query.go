...
type QueryResponse struct {
	...
	// RefusedBy names which guard produced the refusal, empty when Sufficient
	// is true. One of: empty_retrieval, gate, citation, entailment,
	// empty_generation, ablated. It exists so eval can measure whether refusal
	// actually tracks answerability instead of inferring the reason from
	// Warnings[0] as free text.
...
func (a *App) Query(ctx context.Context, req QueryRequest) (QueryResponse, error) {
	...
	if len(stage.Candidates) == 0 {
		...
		resp.RefusedBy = "empty_retrieval"
	...
	if len(ans.Claims) == 0 {
		...
		resp.Answer = insufficientAnswer
		resp.RefusedBy = "empty_generation"
		resp.Timings.Total = ms(start)
		return resp, nil
	}
...
func (a *App) verifyClaims(ctx context.Context, claims []Claim, valid map[string]Source) ([]bool, error) {
	...
	for _, c := range claims {
		...
		// ride straight through into the claim. Escape it too, or the verifier
		// prompt gets the same raw-splice hole the source text just got closed.
		pairs = append(pairs, VerifyPair{Claim: escapeForPrompt(c.Text), Evidence: ev.String()})
	}
...
func (a *App) cachePut(ctx context.Context, qvec []float32, question string, resp QueryResponse) {
	...
	_ = a.Qdrant.Upsert(ctx, a.Cfg.CacheCollection, []qdrant.Point{{
		ID:     qdrant.PointID("cache:" + question),
