...
// ErrIngestBusy is returned by IngestDir when another ingest of the same App
// is already running. Callers such as an HTTP handler translate it into
// whatever status code fits (409 Conflict for POST /ingest).
var ErrIngestBusy = errors.New("an ingest is already running")
...
func (a *App) IngestDoc(ctx context.Context, docID, version string, acl []string, text string) (int, error) {
	...
	pts := make([]qdrant.Point, 0, len(chunks))
...
func (a *App) unchanged(ctx context.Context, docID, version string) bool {
	n, err := a.Qdrant.CountFiltered(ctx, a.Cfg.Collection, qdrant.DocVersionFilter(docID, version))
...
func (a *App) IngestDir(ctx context.Context, dir string, acl []string, force bool) (indexed, skipped, chunks int, err error) {
	...
	for i, p := range paths {
		...
		g.Go(func() error {
			...
			// one representation.
			text, err := extract.LoadDocumentText(docID, raw)
			if err != nil {
...
func SafeCorpusPath(dir, name string) (string, error) {
	...
	if !IsDoc(base) {
		...
		if want, legacy := extract.LegacyOffice[ext]; legacy {
